...
export default function AssetsLayout({ children }) {
  ...
  useEffect(() => {
    if (pathname === '/account/flows') {
      router.replace('/account/flows/forms')
    }
  }, [pathname])
  ...
  // Check if we're in a form builder or booking builder page
  const isInBuilder = pathname.includes('/forms/') && pathname !== '/account/flows/forms'
  
  return (
    <div className={isInBuilder ? '' : 'p-4 sm:p-6'}>
      {/* Only show tabs when not in a builder */}
      {!isInBuilder && (
        <>
          {/* Desktop Tabs */}
