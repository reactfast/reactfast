...
export default function VCardEditor({ params }) {
  ...
  useEffect(() => {
    ...
    if (params.contactId == 'new') {
      setVCardData(template) // is creating new vcard
    } else {
      setVCardData(template) // if editing
    }
  }, [])
  ...
  const saveVCard = async () => {
    ...
    try {
      ...
      setUploadStatus({ success: true, message: 'vCard saved successfully!' })
      router.push('/account/contact-cards')
    } catch (error) {
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1">
            ...
            <button
              onClick={saveVCard}
              className="mb-8 mt-4 flex items-center rounded-md bg-indigo-600 px-4 py-2 text-white shadow hover:bg-indigo-700 focus:ring focus:ring-indigo-500 focus:ring-opacity-50"
            >
          ...
          <div className="pb-8 lg:col-span-2">
            ...
            <pre className="overflow-auto whitespace-pre-wrap rounded-md bg-gray-100 p-4 text-sm text-gray-600">
              {template}
            </pre>
          </div>
        </div>
